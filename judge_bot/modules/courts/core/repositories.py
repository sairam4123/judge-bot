...
from judge_bot.modules.courts.core.models import (
    Case,
    LogEntry,
    Evidence,
    Court,
...
class CaseRepository:
    ...
    def get_case(self, case_id: int) -> Case | None:
        with self.bot.db.get_connection() as conn:
            ...
            case = Case(
                ...
                case_close_reason=row[11] if len(row) > 11 else None,
                case_closed_at=row[12] if len(row) > 12 else None,
            )
            return case
    ...
    def get_witnesses(self, case_id: int) -> list[int]:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT user_id FROM participants WHERE case_id = ? AND role = 'witness'",
                (case_id,),
            )
            row = cursor.fetchall()
            return [r[0] for r in row]
    ...
    def search_cases(self, query: str) -> list[Case]:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM cases WHERE reason LIKE ? OR summary LIKE ? ORDER BY created_at DESC",
                (f"%{query}%", f"%{query}%"),
            )
            rows = cursor.fetchall()
            cases = []
...
class LogRepository:
        ...
        with self.bot.db.get_connection() as conn:
            conn.execute(
                ...
                (
                    ...
                    log_entry.timestamp,
                    log_entry.author_id,
                    int(log_entry.is_judge),
                    log_entry.message,
                    log_entry.message_id,
                    log_entry.message_reference_id,
                    log_entry.summary,
                    log_entry.speaker,
                ),
            )
    ...
    def get_logs(self, case_id: int) -> list[LogEntry]:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM log_entries WHERE case_id = ?", (case_id,)
            )
            rows = cursor.fetchall()
            logs = []
            for row in rows:
    ...
    def count_logs(self, case_id: int) -> int:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT COUNT(message_id) FROM log_entries WHERE case_id = ?",
                (case_id,),
            )
            row = cursor.fetchone()
...
class EvidenceRepository:
    ...
    def get_evidences(self, case_id: int) -> list[Evidence]:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM evidences WHERE case_id = ?", (case_id,)
            )
            rows = cursor.fetchall()
            evidences = []
            for row in rows:
...
class CourtRepository:
    ...
    def has_court(self, court_id: int) -> bool:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT 1 FROM courts WHERE court_id = ?", (court_id,)
            )
            row = cursor.fetchone()
            return row is not None
    ...
    def contains_court(self, guild_id: int, channel_id: int) -> bool:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT 1 FROM courts WHERE guild_id = ? AND channel_id = ?",
                (guild_id, channel_id),
            )
            row = cursor.fetchone()
            return row is not None
...
class CaseParticipantRepository:
    ...
    def add_participant(self, case_id: int, participant: CaseParticipant):
        with self.bot.db.get_connection() as conn:
            conn.execute(
                "INSERT INTO participants (case_id, user_id, role) VALUES (?, ?, ?)",
    ...
    def get_participants(self, case_id: int) -> list:
        with self.bot.db.get_connection() as conn:
            ...
            participants = []
            for row in rows:
                participant = CaseParticipant(user_id=row[1], role=row[2])
...
class AssociatedCaseRepository:
    ...
    def link_associated_cases(self, case_id: int, associated_case_ids: list[int]):
        with self.bot.db.get_connection() as conn:
            conn.execute("DELETE FROM associated_cases WHERE case_id = ?", (case_id,))
    ...
    def get_associated_cases(self, case_id: int) -> list:
        with self.bot.db.get_connection() as conn:
            cursor = conn.execute(
                "SELECT associated_case_id FROM associated_cases WHERE case_id = ?",
                (case_id,),
            )
            rows = cursor.fetchall()
