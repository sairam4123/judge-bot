...
class EvidenceMediaGallery(discord.ui.LayoutView):
    def __init__(
        self,
        content: str,
        evidence_files: Sequence[tuple[discord.File, str | None, str | None]],
    ):
        ...
        images = [
            f
            for f, content_type, _ in evidence_files
            if content_type and content_type.startswith("image/")
        ]
        others = [
            (f, content_type, url)
            for f, content_type, url in evidence_files
        ...
        if others:
            ...
            links = "\n".join(
                [f"[{f.filename}](attachment://{f.filename})" for f, _, _ in others]
            )
