...
from judge_bot.modules.courts.core.prompt import (
    CASE_DETAILS,
    SESSION_MSG,
    SUMMARIZE_PROMPT,
...
type JBToolFunction = t.Callable[t.Concatenate[JudgeBot, ...], t.Awaitable[t.Any]]
type JBTool = tuple[genai_types.Tool, JBToolFunction]


async def google_summarize_file(
    file: discord.Attachment, model: str = JudgeBotConfig().model
) -> str:
    google_client = get_client()
    with open(f"attachments/{file.filename}", "wb") as f:
        await file.save(f)

    uploaded_file = await google_client.aio.files.upload(
        file=f"attachments/{file.filename}"
    )

    while uploaded_file.state == "PROCESSING":
        await asyncio.sleep(5)
        uploaded_file = await google_client.aio.files.get(
            name=f"attachments/{file.filename}"
        )
    ...
    try:
        response = await google_client.aio.models.generate_content(
            model=model,
            contents=[
                "Summarize the contents of the following file:\n\n",
                uploaded_file,
            ],
        )
    except Exception as e:
        print(f"Error occurred while summarizing file: {e}")
...
async def google_summarize_text(text: str, model: str = JudgeBotConfig().model) -> str:
    google_client = get_client()
    response = await google_client.aio.models.generate_content(
        model=model, contents=[SUMMARIZE_TEXT_PROMPT, text]
    )
    if response and response.text:
        return response.text.strip()
    return "Summary could not be generated."
...
async def google_summarize_conversation(
    conversation: list[LogEntry],
    current_summary: str,
    ...
    for evidence in evidences:
        if evidence.summary:
            evidence_summaries.append(
                f"{evidence.filename}: Summary: {evidence.summary}"
            )
...
def tool_declaration(
    ...
    func: JBToolFunction = tool_config.pop("func")
    new_tool_config = genai_types.FunctionDeclaration.model_validate(tool_config)
    return genai_types.Tool(function_declarations=[new_tool_config]), func


def register_tools(
    tools_list: list[dict[str, t.Any]],
) -> list[JBTool]:
    registered_tools = []
